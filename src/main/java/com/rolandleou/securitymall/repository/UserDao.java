package com.rolandleou.securitymall.repository;

import java.util.LinkedHashMap;
import java.util.Map;

import org.springframework.stereotype.Repository;

@Repository
public class UserDao {

	public Map<String, Map<String, String>> users; {
		
		// A01, 1234, admin,normal,ROLE_manager
		Map<String, String> info1 = new LinkedHashMap<>();
		info1.put("password", "$2a$10$VZqAMX90ajOAaVTswq9Eb.s2qq/LCA14ch.0YwmopFjR4TiBdTBOG"); // rong8849
		info1.put("authority", "admin,normal,ROLE_manager");
		
		// A02, 5678, normal,ROLE_employee
		Map<String, String> info2 = new LinkedHashMap<>();
		info2.put("password", "$2a$10$eypWpy03pbo.dfiN1xf0o.esrv9tEeFd8pWkrw4uruqWyPzQGv536"); // rong0560
		info2.put("authority", "normal,ROLE_employee");
		
		users = new LinkedHashMap<>();
		users.put("admin", info1);
		users.put("rolandleou", info2);
		
		System.out.println(users);
	}
}
